...
async function agentExecutor(goal) {
  try {
    ...
    console.log("FastAPI response data:", JSON.stringify(response.data, null, 2));
    ...
    if (error.code === 'ECONNREFUSED') {
      ...
      throw new Error("FastAPI service is not running. Please start the Python server on port 5000.");
    }
    
    if (error.code === 'ETIMEDOUT') {
      console.error("Request timed out - FastAPI is taking too long to respond");
      throw new Error("FastAPI service timed out. The LLM might be taking too long to process.");
    }
    ...
    if (error.response) {
      ...
      throw new Error(`FastAPI error (${error.response.status}): ${error.response.data.detail || error.response.statusText}`);
