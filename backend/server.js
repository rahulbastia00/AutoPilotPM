  ...
  try {
    ...
    if (!isConnected) {
      return res.status(503).json({
        error: "FastAPI service is not available. Please ensure the Python server is running on port 5000.",
...
app.listen(PORT, () => {
  ...
  setTimeout(async () => {
    ...
    if (isConnected) {
      ...
      console.log("❌ FastAPI connection failed - make sure Python server is running");
    }
  }, 1000);
});